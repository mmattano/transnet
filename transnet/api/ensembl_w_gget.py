...
# Constants
from gget.constants import ENSEMBL_REST_API, UNIPROT_REST_API, NCBI_URL
...
def ensembl_get_transcripts(release_number, organism_full):
    ...
    transcript_df = pd.DataFrame(
        ...
        columns=["gene_id", "transcript_id", "transcript_name", "biotype"],
    )
    transcript_df["transcript_name"] = [
        gene.rsplit("-", 1)[0] for gene in transcript_df["transcript_name"]
    ]
...
def _emsembl_expand_with_gget(transcripts, thread_num=1000):
    """Get all the available information connected to Emsembl transcripts using a modified version of gget.
    Includes a sleep of 0.05 seconds between each thread to avoid overloading the server.
    
    Parameters
    ----------
    transcripts : pd.DataFrame
        Data frame of transcripts, e.g. from ensembl_get_transcripts().
    thread_num : int, optional
        Number of threads to use, by default 1000, using more only at your own risk.
    
    Returns
    -------
    results : list
        List of dictionaries with information about transcripts."""

    transcripts = transcripts["gene_id"].tolist()
    if thread_num > len(transcripts):
        thread_num = len(transcripts)
        print(
            "thread_num is larger than the number of transcripts, thread_num is set to the number of transcripts"
        )

    block_len = len(transcripts) // thread_num
    difference = len(transcripts) - block_len * thread_num

    if difference > 0:
        thread_num += 1

    def func_to_run(transcripts_block, results, index):
        results[index] = _gget_info_mod(transcripts_block, verbose=False)
    ...
    for i in range(thread_num):
        if difference > 0:
            if i == thread_num - 1:
                x = threading.Thread(
                    target=func_to_run,
                    args=(transcripts[-difference:], results, i),
                )
            else:
                x = threading.Thread(
                    target=func_to_run,
                    args=(
                        transcripts[block_len * i : block_len * (i + 1)],
                        results,
                        i,
                    ),
                )
...
def emsembl_gget_expander(transcripts, thread_num=1000):
    """Get all the available information connected to Emsembl transcripts using gget.
    Calls _emsembl_expand_with_gget() multiple times to avoid overloading the server.
    Only calls one transcript per thread since the time to get the information increases linearly with the number of transcripts.
    ...
    transcripts = transcripts["gene_id"].tolist()
    start = 0
    interval_size = thread_num  # round(len(transcripts) / thread_num)
    stop = interval_size
    counter = 1
    results = _emsembl_expand_with_gget(
        transcripts[start:stop], thread_num=len(transcripts[start:stop])
    )
    while True:
        start += interval_size
        stop += interval_size
        if start >= len(transcripts):
            break
        elif stop >= len(transcripts):
            stop = len(transcripts)
        results += _emsembl_expand_with_gget(
            transcripts[start:stop], thread_num=len(transcripts[start:stop])
        )
        counter += 1
...
def ensemble_gapfiller(transcripts, results, iterations=20, thread_num=1000):
    """Fill in the gaps in the results from emsembl_gget_expander().
    Some IDs are not found by gget right away due to communication issues with the server.
    This function calls gget again for the IDs that were not found in the first iteration.
    ...
    transcripts : pd.DataFrame
        Data frame of transcripts, e.g. from ensembl_get_transcripts().
    results : list
        List of dictionaries with information about transcripts, e.g. from emsembl_gget_expander().
    iterations : int, optional
        Number of iterations to run, by default 20.
    thread_num : int, optional
        Number of threads to use, by default 1000, using more only at your own risk.
    ...
    output_df : pd.DataFrame
        Dataframe with all the information about the transcripts.
    """

    transcripts = transcripts["gene_id"].tolist()
    output_df = pd.concat(results, axis=0)
    i = 0
    rerun_transcripts = transcripts
    while i < iterations:
        if any([True for result in results if result is None]):
            rerun_transcripts = [
                rerun_transcripts[i]
            ...
            print(
                f"No results for {len(rerun_transcripts)} transcripts, rerunning gget for them."
            )
            results = _emsembl_expand_with_gget(rerun_transcripts, thread_num)
            if not all([result is None for result in results]):
                output_df = pd.concat(
                    [output_df, pd.concat(results, axis=0)], axis=0
                )
        i += 1
    if not any([True for result in results if result is None]):
        output_df = output_df.reindex(transcripts)
...
def _gget_info_mod(
    ...
    if ensembl_only is False:
        ...
        for ens_id in ens_ids_clean_2:
            ...
            if ncbi_synonyms is not None and not isinstance(
                df_uniprot, type(None)
            ):
            ...
            elif ncbi_synonyms is None and not isinstance(
                df_uniprot, type(None)
            ):
    ...
    df_final = pd.concat(
        [
            df_final,
            pd.DataFrame.from_dict(data, orient="index", columns=ens_ids),
        ]
    )
    ...
    if wrap_text:
        ...
        wrap_cols_func(
            df_wrapped, ["uniprot_description", "ensembl_description"]
        )
    ...
    if json:
        ...
        if save:
            with open("gget_info_results.json", "w", encoding="utf-8") as f:
                json_package.dump(
                    results_dict, f, ensure_ascii=False, indent=4
                )
    ...
    if ensembl_only is False:
        ...
        for ens_id in ens_ids_clean_2:
            ...
            if ncbi:
                ...
                # Check if NCBI gene ID is available
                try:
                    ncbi_gene_id = soup.find(
                        "input", {"id": "gene-id-value"}
                    ).get("value")
                except:
                    ncbi_gene_id = np.nan
