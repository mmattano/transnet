        ...
        for dataset in datasets:
            ...
            for dl in what:
                ...
                try:
                    ...
                    if what is not None:
                        raise Exception(
                            "Error downloading from %s: %s" % (url, e)
                        )
...
def _parse(
    ...
    with open(fn, "r", newline=row_sep, **open_args) as fh:
        header = np.array(
            [next(fh).strip().split(col_sep) for _ in range(column_size)]
        )
...
def _parse_df(
    ...
    **kwargs
    ...
    (column_names, columns, index_names, index, data,) = _parse(
        fn, data_fmt=data_fmt, **kwargs
    )
    ...
    return df_type(
        ...
        index=pd.Index(data=index, name=str(index_names), dtype=np.object,),
        columns=pd.Index(
            data=columns, name=str(column_names), dtype=np.object,
        ),
        **df_args,
    )
